...
void quat_multiply(float* q_left, float* q_right, float* resulting_quat) {
    resulting_quat[0] = q_left[0] * q_right[0] - q_left[1] * q_right[1] - q_left[2] * q_right[2]
                        - q_left[3] * q_right[3];
    resulting_quat[1] = q_left[0] * q_right[1] + q_left[1] * q_right[0] + q_left[2] * q_right[3]
                        - q_left[3] * q_right[2];
    resulting_quat[2] = q_left[0] * q_right[2] - q_left[1] * q_right[3] + q_left[2] * q_right[0]
                        + q_left[3] * q_right[1];
    resulting_quat[3] = q_left[0] * q_right[3] + q_left[1] * q_right[2] - q_left[2] * q_right[1]
                        + q_left[3] * q_right[0];
}
...
void rotationvec2quat(float* vec, float* resulting_quat) {
    float angle = sqrtf(pow(vec[0], 2) + pow(vec[1], 2) + pow(vec[2], 2));

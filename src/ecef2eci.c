...
float32_t jd_2_gmst(float32_t jd) {
    ...
    float32_t theta_deg = 280.46061837 + 360.98564736629 * (jd - 2451545.0) + 0.000387933 * t * t
                          - (t * t * t) / 38710000.0;
...
void ecef_2_eci(float* ecef, float* eci, int unix) {
    ...
    float32_t R_data[] = {cosf(theta), -sinf(theta), 0, sinf(theta), cosf(theta), 0, 0, 0, 1};
    const arm_matrix_instance_f32 R = {3, 3, (float32_t*)R_data};
    const arm_matrix_instance_f32 ecef_mat = {3, 1, ecef};
    arm_matrix_instance_f32 eci_mat = {3, 1, eci};

    arm_mat_mult_f32(&R, &ecef_mat, &eci_mat);
}
